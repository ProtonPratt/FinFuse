...
def run_fused_lstm_text_classification(
    ...
    # FinBERT Sentiment
    finbert_sentiment_model_name = "ProsusAI/finbert"
    print(f"Initializing FinBERT Sentiment Analyzer ({finbert_sentiment_model_name})...")
    finbert_analyzer = SentimentAnalyzer(model_name=finbert_sentiment_model_name, device=DEVICE)
    ...
    if not results_df.empty:
        ...
        results_df.to_csv(RESULTS_DIR / f"fused_lstm_text_models_summary_lag{target_lag_days}_hist{history_lags_days_str}.csv", index=False)
    else:
